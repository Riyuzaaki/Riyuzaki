...
<section id='services'>
    <div class='container'>
     ...
     <?php  
        $searchres = mysqli_real_escape_string($conn,$_POST['search']);
    $sql = "SELECT * FROM product WHERE pdname LIKE '%$searchres%' OR modelnum LIKE '%$searchres%' OR pdtype LIKE '%$searchres%' OR pddes LIKE '%$searchres%'";
    ...
    if(mysqli_num_rows($result) > 0)
    {
        while($row = mysqli_fetch_assoc($result))
        {
            ...
            {
                 echo "<h1 class='resultsearch'>No Result Found</h1>";
            }
?>
        </div>
        </div>
